...
class TrackMatcher:
    """
    Heuristics scoring engine to rank provider search candidates against the original track query.
    """
    ...
    def find_best_match(original: Track, candidates: List[CandidateTrack]) -> CandidateTrack:
        ...
        if not ranked:
            raise ValueError(f"No candidate tracks available to match for: '{original.name}'")
        return ranked[0]

    @staticmethod
    def rank_candidates(original: Track, candidates: List[CandidateTrack]) -> List[CandidateTrack]:
        """
        Scores all candidate tracks and returns them sorted descending by calculated match score.
        """
        if not candidates:
            return []

        scored = [
            (candidate, TrackMatcher._calculate_score(original, candidate))
    ...
    def _calculate_score(original: Track, candidate: CandidateTrack) -> int:
        ...
        if original.duration_ms > 0 and candidate.duration_ms > 0:
        ...
        unwanted = [
            'live', 'cover', 'karaoke', 'remix', 'tribute',
            'instrumental', 'parody', 'acoustic', 'slowed', 'reverb'
        ]
        for kw in unwanted:
        ...
        if primary_artist and (
            primary_artist in candidate.artist.lower()
            or candidate.artist.lower() in primary_artist
        ):
            score += 20
